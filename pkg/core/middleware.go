...
func AddTrace() gin.HandlerFunc {
	return func(context *gin.Context) {
		...
		repeatCheck := context.Query("__check__")
		if repeatCheck == "check" && traceIdQuery == "" {
...
// 日志中间件
func AccessLog(god *GCore) gin.HandlerFunc {
...
// 异常恢复中间件
func CustomRecover(god *GCore) gin.HandlerFunc {
	return func(c *gin.Context) {
		defer func() {
