...
func UpdateItem[T any](db *gorm.DB, obj T, id uint) error {
	var model T
	return db.Model(model).Where("id=?", id).Debug().Updates(obj).Error
}
func DeleteItem[T any](db *gorm.DB, id uint) error {
	var model T
	return db.Model(model).Where("id=?", id).Debug().Delete(&model).Error
}
...
func GetItemByID[T any](db *gorm.DB, id int, call DBChange) (T, error) {
	var res T
	if call != nil {
		db = call(db)
	}
...
func GetItem[T any](db *gorm.DB, id uint, call DBChange) (T, error) {
	var res T
	var model T
	db = db.Model(model)
	...
	err := db.Debug().Where("id=?", id).First(&res).Error
...
func GetItemByFunc[T any](db *gorm.DB, call DBChange) (T, error) {
	var res T
	var model T
	db = db.Model(model)
